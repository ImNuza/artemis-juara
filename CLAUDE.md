...
6. **Short overlay** -- tested May 7 and rejected (Sharpe collapse, large DD blow-up against the May-7 baseline). Re-tested May 10 against the then-current 1.31 baseline: shorting bottom-2 during BEAR (Sharpe 0.10, DD -86.1%, equity 1.38x) or top-2 during BEAR (Sharpe 0.05, DD -81.7%, equity 1.18x) both destroyed capital. Code was a manual one-off test and is not in the repo; the numbers here are from the session log. Documented as honest negative finding. The qualitative result (short overlay destroys capital) has held across every baseline tested since.
...
- Git initialized, committed, pushed to public GitHub (`github.com/ImNuza/artemis-juara`)
- Research report PDF rendered (`docs/research_report.pdf`)
- Pitch deck PPTX built (`docs/Artemis_Pitch_Deck.pptx`)
- All analysis scripts verified, headline numbers confirmed
- Data integrity audit passed

### Remaining

- Export PPTX to Google Slides and share with `lindsey@artemisanalytics.xyz`
- Send submission email with PDF + repo URL + deck URL
