...
Threshold 55 produces a higher raw Sharpe (1.46 vs 1.31) but at -35.1% drawdown vs -27.2%. We ship 60 ,  ImNuza's pre-classified value ,  because the lower drawdown is more valuable than the marginal Sharpe gain for a strategy positioned around risk control.
...
Revenue growth and activity momentum were removed May 7, 2026 after factor attribution showed they each contributed +0.01 Sharpe (negligible marginal value). They only had data for 2 of 7 assets (SOL and HYPE). The remaining two factors account for the full signal:

| Factor | Weight | Signal | Available For |
|--------|--------|--------|---------------|
| Funding Rate (inverted) | 45% | Weekly HL funding (Bybit-spliced for XMR pre-HL-listing; HIP-3 funding for equities from listing), negative = bullish | All 7 assets where data exists |
...
The strategy beats both benchmarks on every metric ,  higher final equity than EW Alts (5.63x vs 4.41x) with dramatically lower drawdown (-27.2% vs -66.6%). Calmar (1.79 vs 0.61) is the cleaner comparison. The regime gate buys risk-adjusted return, not raw return.
...
| Bybit API | XMR linear-perp funding rate pre-HL-listing (before 2026-01-16) | Weekly |
...
- [x] Files staged to git (`git add`); author identity config needed before commit

**Outstanding (submission blockers)**
- [ ] **Configure git author, commit, and push to public GitHub repo.** Files staged; needs `git config user.email/name` then commit. Repo URL goes in the email.
- [ ] **Render `docs/research_report.md` as PDF.** Pandoc + LaTeX. PDF is the email attachment.
- [ ] **Build pitch deck (Google Slides).** Not started. Share with `lindsey@artemisanalytics.xyz`. ~10 to 12 slides mapping to `research_report.md` sections.
